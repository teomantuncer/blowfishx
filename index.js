...
crypto.Blowfish = new function () {
	...
	let iv = null 	//	CBC mode initialization vector
	const boxes = {
		p: [
	...
	this.getIV = function (/* dojox.encoding.crypto.outputTypes? */ outputType) {
		...
		const out = outputType || crypto.outputTypes.Base64
		switch (out) {
	...
	this.setIV = function (/* string */data, /* dojox.encoding.crypto.outputTypes? */inputType) {
		...
		const ip = inputType || crypto.outputTypes.Base64
		let ba = null
		switch (ip) {
			case crypto.outputTypes.String: {
	...
	this.encrypt = function (/* string */plaintext, /* string */key, /* object? */ao) {
		...
		let out = crypto.outputTypes.Base64
		let mode = crypto.cipherModes.ECB
		if (ao) {
			if (ao.outputType) {
				out = ao.outputType
			}
	...
	this.decrypt = function (/* string */ciphertext, /* string */key, /* object? */ao) {
		...
		let ip = crypto.outputTypes.Base64
		let mode = crypto.cipherModes.ECB
		if (ao) {
			if (ao.outputType) {
				ip = ao.outputType
